...
const AdminDashboard: React.FC = () => {
  ...
  const savePaymentSettings = () => {
    saveToStorage('vault_qr_code', convertToDirectLink(siteQr));
      ...
      <main className="flex-grow p-4 md:p-12 overflow-y-auto">
          ...
          <div className="max-w-3xl space-y-8">
            ...
            <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm space-y-6">
              <div className="space-y-4">
                <input value={recipientName} onChange={e => setRecipientName(e.target.value)} className="w-full p-4 bg-slate-50 border rounded-xl font-bold" placeholder="Name" />
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase text-slate-400">QR Image Link (Direct)</label>
                  <input value={siteQr} onChange={e => setSiteQr(e.target.value)} className="w-full p-4 bg-slate-50 border rounded-xl font-bold" placeholder="QR URL" />
                </div>
                <textarea value={instructions} onChange={e => setInstructions(e.target.value)} rows={3} className="w-full p-4 bg-slate-50 border rounded-xl font-bold" />
              </div>
              <button onClick={savePaymentSettings} className="w-full bg-indigo-600 text-white py-5 rounded-2xl font-black uppercase">Save Payment Config</button>
            </div>
          </div>
        )}
        ...
        {activeTab === 'config' && (
          <div className="bg-white p-8 rounded-[2rem] border border-slate-200 space-y-6 max-w-2xl">
            ...
            <input value={siteBanner} onChange={e => setSiteBanner(e.target.value)} className="w-full p-4 bg-slate-50 border rounded-xl font-bold" />
            <div className="aspect-[12/5] border rounded-2xl overflow-hidden">
