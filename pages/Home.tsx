...
const Home: React.FC<Props> = ({ addToCart }) => {
  ...
  return (
    <div className="animate-in fade-in duration-500 overflow-x-hidden w-full bg-white min-h-screen flex flex-col">
      {/* Banner Section - Full width, No gap, No box style */}
      <section className="relative w-full">
        <div className="w-full aspect-[12/5] relative overflow-hidden flex items-center justify-center">
          <img 
            src={banner} 
            alt="Hero Banner"
      ...
      <div className="py-8 px-4 mb-10">
        <div className="max-w-4xl mx-auto rounded-3xl md:rounded-[4rem] p-8 md:p-20 bg-slate-900 text-center border border-white/5 flex flex-col items-center gap-6 shadow-2xl relative overflow-hidden">
          <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-500/10 rounded-full blur-3xl"></div>
          <h3 className="text-xl md:text-5xl font-[1000] text-white uppercase tracking-tighter italic relative z-10">
            Join Our <span className="text-indigo-400">Success</span> Community
          </h3>
          <p className="text-slate-400 font-bold uppercase tracking-widest text-[8px] md:text-xs relative z-10">Need help? Our team is available 24/7 via Email.</p>
          <Link 
            to="/contact" 
            className="px-6 py-3.5 md:px-12 md:py-6 bg-indigo-600 text-white rounded-xl md:rounded-[2rem] font-black uppercase text-[10px] md:text-lg tracking-[0.2em] shadow-xl hover:bg-indigo-500 active:scale-95 transition-all relative z-10"
          >
            Contact Support
          </Link>
        </div>
